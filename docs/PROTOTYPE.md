...
- [0. Architecture Diagrams](#0-architecture-diagrams)
  - [0.1 Data flow between the 3 CSVs](#01-data-flow-between-the-3-csvs)
  - [0.2 Processing pipeline](#02-processing-pipeline)
  - [0.3 Complete MVC architecture](#03-complete-mvc-architecture)
  - [0.4 Advisor query flow](#04-advisor-query-flow)
  - [0.5 Training pipeline (SVG)](#05-training-pipeline-svg)
  - [0.6 RAG pipeline (SVG)](#06-rag-pipeline-svg)
...
0.1 Data flow between the 3 CSVs
...
sequenceDiagram
    ...
    A->>RV: retrieve(question + route + airline)
    RV->>FS: exact match by metadata\n(origin, dest, carrier) k=2\n+ semantic search k=4
    FS-->>RV: relevant docs
    RV-->>A: context (~800 tokens)

    A->>Q: prompt = system + flight_data\n+ ml_prediction + shap_factors\n+ rag_context + instructions
    Q-->>A: advice (natural language)

    A-->>D: {delay_probability, risk_level,\ntop_factors, advice}
    D-->>U: Risk badge + SHAP chart\n+ Advisor response
```

### 0.5 Training pipeline (SVG)

<img width="640" height="332" alt="image" src="https://github.com/user-attachments/assets/5c819dd1-81f3-4f15-a480-1a0d6ccc0fae" />



### 0.6 Pipeline - RAG (SVG)

<img width="636" height="360" alt="image" src="https://github.com/user-attachments/assets/6b2da9db-3b73-4889-b57e-b2a1042f7a40" />
