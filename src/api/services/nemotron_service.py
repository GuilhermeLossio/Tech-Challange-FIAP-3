...
def _provider_config() -> LLMProviderConfig:
    ...
    if not model:
        ...
        raise RuntimeError("Nemotron model is not configured.")
...
def generate_nemotron_advice_stream(
    context: dict[str, Any],
    history: list[dict[str, Any]] | None = None,
    system_prompt: str | None = None,
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            if response.status_code in RETRY_STATUS_CODES and attempt < attempts:
                ...
                logger.warning(
                    "Nemotron retornou HTTP %d (tentativa %d/%d). Aguardando %.1fs...",
        ...
        logger.info("%s advice gerado com modelo %s (tentativa %d/%d)", config.label, config.model, attempt, attempts)
        return NemotronAdvice(content=content, provider=config.label, model=config.model)

    raise RuntimeError(f"{config.label} falhou apos {attempts} tentativas.") from last_exc
